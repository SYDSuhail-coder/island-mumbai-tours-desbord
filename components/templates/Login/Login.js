...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const url = `/api/login-create-api`;
    await axios.post(url, formData).then(() => {
      router.push('/loginAlready')
    })
    setName("");
    setEmail("");
    setNumber("");
      ...
      <div className="w-full max-w-md bg-gray-800 rounded-2xl shadow-xl ring-1 ring-emerald-100 p-8">
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            <div className="relative">
              <Phone className="absolute left-3 top-2.5 h-5 w-5  text-white" />
              <input
                type="tel"
                required
                value={number}
        ...
        <p className="text-sm text-center text-white mt-6">
          ...
          <a href="/loginAlready" className="font-semibold text-emerald-400 hover:underline">
            loginAlready
          </a>
        </p>
      </div>
